...
def extract_order_id(text):
    match = re.search(r'Order\s*ID\s*:\s*([A-Z0-9\-]+)', text, re.IGNORECASE)
    if match:
        return match.group(1).strip()
    return "Unknown"

# ================= FIXED SKU (ดึงตัวแรกสุดที่เจอเสมอ) =================
def extract_sku(text):
    ...
    for pattern in patterns:
        # re.search จะสแกนและดึงเฉพาะตัวแรกสุดที่เจอ ทำให้ใบที่มีหลายโค้ด จะถูกเรียงด้วยโค้ดหลักตัวแรก
        match = re.search(pattern, text)
        if match:
            return match.group(1)
    return "ZZZZZZ"

# ================= FIXED QTY (ยึด "รวมทั้งสิ้น" เป็นหลัก) =================
def extract_qty(text):
    # รีดข้อความให้เป็นบรรทัดเดียว ป้องกันคำว่า "รวมทั้งสิ้น" โดนแยกบรรทัดกับตัวเลข
    full_text = text.replace("\n", " ").replace("\r", " ")
    
    # 1. หาคำว่ารวมทั้งสิ้นก่อน (แม่นยำที่สุดตามที่คุณกุลวัชรแนะนำ)
    total_match = re.search(r'รวมทั้งสิ้น\s*(\d{1,3})', full_text)
    if total_match:
        return int(total_match.group(1))
        
    # 2. ถ้าไม่มีคำว่ารวมทั้งสิ้น ลองหาคำว่า จำนวน หรือ Qty
    fallback_match = re.search(r'(?:จำนวน|Qty|Quantity)\s*[:=]?\s*(\d{1,3})', full_text, re.IGNORECASE)
    if fallback_match:
        return int(fallback_match.group(1))

    # ถ้าหาไม่เจอจริงๆ ให้ตีเป็น 1 ชิ้น
    return 1
...
def process_multiple_pdfs(uploaded_files, sort_mode):
    ...
    if sort_mode == "🚚 เรียงตามขนส่ง -> SKU":
        all_pages_data.sort(key=lambda x: (x["courier"], x["zone"], x["sku"]))
...
with col_left:
    ...
    st.caption("ระบบจัดเรียงบิลอัจฉริยะสำหรับคลังสินค้า")

with col_right:
    # ดึงรูปภาพพนักงานคลังสินค้ากลับมาแสดงผลที่มุมขวาบน
    try:
...
if uploaded_files:
    ...
    if st.button("⚡ เริ่มจัดบิล", type="primary", use_container_width=True):
        try:
            ...
            display_df.columns = ["หน้า", "ขนส่ง", "โซน", "SKU", "จำนวน", "Order ID"]

            search = st.text_input("ค้นหา SKU / Order ID / ขนส่ง")
            if search:
                filtered = display_df[
