...
# --- ฟังก์ชันหลักดึงและคัดแยกข้อมูลบิล (ย้อนกลับไปใช้เวอร์ชันแรกสุด) ---
...
def extract_data_from_page(text):
    ...
    # ดึง SKU แบบเวอร์ชันแรก (ค้นหารหัสแรกสุดในหน้า แล้วจบเลย)
    ...
    # ดึงจำนวน (ใช้วิธีตรงไปตรงมาแบบเวอร์ชันแรก จับคำว่า รวมทั้งสิ้น)
    # เพิ่ม .replace('\n', ' ') นิดเดียวเพื่อป้องกันคำขาดบรรทัด
...
with col_right:
    # ดึงรูปภาพที่คุณอัปโหลดไว้กลับมาเหมือนเดิม
    try:
...
if uploaded_files:
    ...
    if st.button("⚡ เริ่มจัดบิลและสรุปยอดรวม", type="primary", use_container_width=True):
        with st.spinner("⏳ ระบบกำลังผสานไฟล์... กรุณารอสักครู่"):
            try:
                ...
                st.download_button(
                    label="📥 1. ดาวน์โหลดไฟล์บิล PDF ที่มัดรวมและจัดเรียงใหม่ทั้งหมด",
                    data=sorted_pdf,
